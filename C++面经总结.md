...
4. 什么是内存池，如何实现
    ...
    - 内存池实现机制
        ...
        首先用malloc()配置一定数量的区块（40个），一半留给内存池，一半给程序使用。如果客户端还有内存需求，再调用malloc()配置空间，此时新申请的区块数量会随着调用次数递增，同样一半提供程序使用，另一半留给内存池。如果整个堆的空间都不够了，就会在原先已经分配区块中寻找能满足当前需求的区块数量，能满足就返回，不能满足就向客户端报bad_alloc异常。
...
3. C++中新增了string，它与C语言中的 char *有什么区别吗？它是如何实现的？
