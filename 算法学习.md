...
3. 动态规划的Debug
   ...
   - 打印dp数组，其是否和预想的一致，如果打印出来和自己预先模拟推导是一样的，那么就是自己的递归公式、初始化或者遍历顺序有问题了。
      `printf("%d\n", dp[i]);`
4. 注意
   - 注意根据dp的定义的初始化，并且循环要从已经初始化的后一位开始！
...
1. 完全背包理论基础
   ...
   - 注意背包问题一般定义dp时是nums.size() + 1
   - 注意背包问题对于计算种类的题型，初始化是dp[0] = 1
   - 注意对于初始化是INT_MAX的类型要考虑越界，以及可能需要dp[0] = 0等等
2. 组合问题：518零钱兑换 II
   - 同样dp[j] += dp[j - coins[i]]
...
#### 12.7 多重背包
1. 多重背包可以转化为01背包
2. 将nums[i] > 1的展开，同时weight.push_back(weight[i]);value.push_back(value[i]);
...
1. 198打家劫舍
   ...
   - 注意这题dp[i]i代表的是下标，从0开始，所以 vector<int> dp(nums.size(), 0);
   - 特殊情况：nums.size() == 0，nums.size() == 1
   - 注意：不偷i时，并不代表一定要偷i - 1
2. 213打家劫舍 II
   - 两种情况
     - 不偷第0个，偷窃范围为1 ~  nums.size() - 1
     - 不偷最后一个， 偷窃范围为0 ~ nums.size() - 2
   - 实现函数：robRange(const vector<int>& nums, int start, int end)
     - 注意要考虑start==end的情况
     - 最后返回的为dp[end]
3. 树形动规：337打家劫舍 III
...
#### 12.9 子序列/子数组问题
1. 300最长递增子序列
   - dp[i]：i之前包括i的以nums[i]结尾最长上升子序列的长度
   - if(nums[i] > nums[j]) dp[i] = max(dp[i], dp[j] + 1);
   - 初始化为1
   - 由于是要求最长，所以result要在循环内比较得出，result = max(result, dp[i]);
...
3. 718最长重复子数组
   ...
   - dp[i][j]：长度分别为i，j即下标为i - 1， j - 1的数组最长重复子数组的长度，注意这个子数组必须包括当前值！
   -  if(nums1[i - 1] == nums2[j - 1]) dp[i][j] = dp[i - 1][j - 1] + 1;

4. 1143最长公共子序列
   - 与718区别在于不连续
   - dp[i][j]:长度为[0, i - 1]的字符串text1与长度为[0, j - 1]的字符串text2的最长公共子序列为dp[i][j]
   - 当前比较不相同时， dp[i][j] = max(dp[i - 1][j], dp[i][j - 1]);
5. 1035不相交的线
   - 要求最大连线数，实际上要求的就是最长的公共子序列，即既要相等，又不能改变顺序
   - 同1143
6. 53最大子数组和
   - 这题也可以用贪心做，但是还是dp比较容易理解
   - dp[i]下标为i的连续子数组的最大和
