...
3. 空闲链表法
   ...
   - 优点：简单，只要在主存中保存⼀个指针，令它指向第⼀个空闲块。
   - 缺点
     - 不能随机访问
     - ⼯作效率低，因为每当在链上增加或移动空闲块时需要做很多 I/O 操作
     - 同时数据块的指针消耗了⼀定的存储空间
     - 不适合⽤于⼤型⽂件系统，会使得空闲链表太大
4. 位图法
   - 利⽤⼆进制的⼀位来表示磁盘中⼀个盘块的使⽤情况，磁盘上所有的盘块都有⼀个⼆进制位与之对应。当值为 0 时，表示对应的盘块空闲，值为 1 时，表示对应的盘块已分配。
   - 在 **Linux ⽂件系统**就采⽤了位图的⽅式来管理空闲空间，不仅⽤于数据空闲块的管理，还⽤于 inode 空闲块的管理，因为 inode 也是存储在磁盘的，⾃然也要有对其管理。
#### 6.6 文件系统的结构
1. 单块位图的问题
      如果采⽤「⼀个块的位图 + ⼀系列的块」，外加「⼀个块的 inode 的位图 + ⼀系列的 inode 的结构」能表示的最⼤空间也就 128M
2. 采用多快组结构表示文件
   - ⽂件系统都由⼤量块组组成，在硬盘上相继排布
   ...
   - 块组构成
     ...
     - 数据块，包含⽂件的有⽤数据。
3. 快组内信息重复
   - 超级块和块组描述符表，这两个都是全局信息，⽽且⾮常的重要
   - 如果系统崩溃破坏了超级块或块组描述符，有关⽂件系统结构和内容的所有信息都会丢失。如果有冗余的副本，该信息是可能恢复的。
   - 通过使⽂件和管理数据尽可能接近，减少了磁头寻道和旋转，这可以提⾼⽂件系统的性能。
   - Ext2 的后续版本采⽤了稀疏技术，超级块和块组描述符只储存再部分快组中

#### 6.7 目录的存储
1. 与普通文件的不同
   - ⽬录其实也是个⽂件，也有 inode，inode ⾥⾯也是指向⼀些块
   - 普通⽂件的块⾥⾯保存的是⽂件数据，⽽⽬录⽂件的块⾥⾯保存的是⽬录⾥⾯⼀项⼀项的⽂件信息
2. 目录文件块中的保存格式
   - 列表
     - ⼀项⼀项地将⽬录下的⽂件信息（如⽂件名、⽂件inode、⽂件类型等）列在表⾥
     - 列表中每⼀项就代表该⽬录下的⽂件的⽂件名和对应的 inode，通过这个 inode，就可以找到真正的⽂件
     - 第⼀项是「 . 」，表示当前⽬录，第⼆项是「 .. 」，表示上⼀级⽬录
     - 缺点：目录文件多时，效率不高
   - 哈希表
     - 保存⽬录的格式改成哈希表，把文件名对应的哈希值保存。查找文件时，通过文件名取哈希值。如果匹配上，就说明这个⽂件的信息在相应的块⾥⾯。
     - 优点：查找⾮常迅速，插⼊和删除也较简单
     - 缺点：要处理哈希冲突
   - 当前使用目录缓存在内存
...
1. 硬链接（Hard Link）
   ...
   - 硬链接是不可⽤于跨⽂件系统的，每个⽂件系统都有各⾃的 inode 数据结构和列表
   - 由于多个⽬录项都是指向⼀个 inode，那么只有删除⽂件的所有硬链接以及源⽂件时，系统才会彻底删除该⽂件
2. 软链接Symbolic Link）
   - 相当于重新创建⼀个⽂件，这个⽂件有独⽴的 inode，这个⽂件的内容是另外⼀个⽂件的路径
   - 访问软链接的时候，实际上相当于访问到了另外⼀个⽂件，所以软链接是可以跨⽂件系统的，甚⾄⽬标⽂件被删除了，链接⽂件还是在的，只不过指向的⽂件找不到了⽽已

#### 6.9 文件I/O
1. 缓冲与⾮缓冲 I/O
   - ⽂件操作的标准库是可以实现数据的缓存，那么根据「是否利⽤标准库缓冲」，可以把⽂件 I/O 分为缓冲I/O 和⾮缓冲 I/O
   - 缓存I/O
      利⽤的是标准库的缓存实现⽂件的加速访问，⽽标准库再通过系统调⽤访问⽂件
   - ⾮缓冲 I/O
      直接通过系统调⽤访问⽂件，不经过标准库缓存
   - 这⾥所说的「缓冲」特指标准库内部实现的缓冲。
   - 作用
      减少系统调⽤的次数，即减少CPU 上下⽂切换的开销。比如，减少系统调⽤的次数，毕竟系统调⽤是有 CPU 上下⽂切换的开销的
2. 直接与⾮直接 I/O
   ...
   - 直接 I/O
      不会发⽣内核缓存和⽤户程序之间数据复制，⽽是直接经过⽂件系统访问磁盘
   - ⾮直接 I/O
      读操作时，数据从内核缓存中拷⻉给⽤户程序，写操作时，数据从⽤户程序拷⻉给内核缓存，再由内核决定什么时候写⼊数据到磁盘
   ...
   - 以下⼏种场景会触发内核缓存的数据写⼊磁盘
     ...
     - 内核缓存的数据的缓存时间超过某个时间时，也会把数据刷到磁盘
3. 阻塞与⾮阻塞 I/O VS 同步与异步
   - 同步I/O
     - 阻塞I/O
     - 非阻塞I/O
        一直问询，循环
     - 基于非阻塞I/O的多路复用
        select、 poll，它是通过 I/O 事件分发，当内核数据准备好时，再以事件通知应⽤程序进⾏操作
   - 异步I/O
     - 异步I/O(aio_read)
        数据未准备好立即返回
   - I/O过程
...
#### 8.1 直接内存访问DMA（Direct Memory Access）
1. 概念
    在进⾏ I/O 设备和内存的数据传输的时候，数据搬运的⼯作全部交给DMA 控制器，⽽ CPU 不再参与任何与数据搬运相关的事情，这样 CPU 就可以去处理别的事务。防止大数据传输时占用CPU
2. 具体过程
   ...
   - 当 DMA 读取了⾜够多的数据，就会发送中断信号给 CPU；CPU 收到 DMA 的信号，知道数据已经准备好，于是将数据从内核拷⻉到⽤户空间，系统调⽤返回；

#### 8.2 零拷贝
1. 文件传输过程中发生了四次拷贝
   - DMA搬运：磁盘上的数据拷⻉到内核的缓冲区
   - CPU搬运：内核缓冲区的数据拷⻉到⽤户的进程缓冲区⾥
   - CPU搬运：⽤户的缓冲区⾥的数据，拷⻉到内核的socket的缓冲区
   - DMA搬运：内核的socket缓冲区⾥的数据，拷⻉到⽹卡的缓冲区⾥
2. 如何提高文件传输的性能
   - 减少⽤户态与内核态的上下⽂切换
     - 用户空间没有权限操作磁盘或网卡等硬件，需要使用内核的系统调用函数，而一次系统调用会发生2次上下文切换：⾸先从⽤户态切换到内核态，当内核执⾏完任务后，再切换回⽤户态交由进程代码执⾏
     - 要想减少上下⽂切换的次数，就要减少系统调⽤的次数
   - 减少内存拷⻉的次数
     - 实际传输中在⽤户空间我们并不会对数据再加⼯，所以数据实际上可以不⽤搬运到⽤户空间，因此⽤户的缓冲区是没有必要的
3. 零拷贝（Zero-copy）
   - mmap() + write()
     - ⽤ mmap() 替换 read() 系统调⽤函数，直接把内核缓冲区⾥的数据「映射」到⽤户空间
     - 可以减少⼀次数据拷⻉的过程，
     - 不是最理想，因为仍然需要通过 CPU 把内核缓冲区的数据拷⻉到 socket 缓冲区⾥，⽽且仍然需要 4 次上下⽂切换，因为系统调⽤还是 2 次：mmap(),write()
   - sendfile()
   ...
   - SG-DMA拷贝（The Scatter-Gather Direct Memory Access）
     - ⽹卡的 SG-DMA 控制器就可以直接将内核缓存中的数据拷⻉到⽹卡的缓冲区⾥，此过程不需要调用CPU将数据从操作系统内核缓冲区拷⻉到 socket缓冲区中
     - 减少了⼀次数据拷⻉
     - 只进⾏了 2 次数据拷⻉,2次上下⽂切换
     - 这就是所谓的零拷⻉（Zero-copy）技术，因为我们没有在内存层⾯去拷⻉数据，也就是说全程没有通过CPU 来搬运数据，所有的数据都是通过 DMA 来进⾏传输的。 
     - 例： Kafka，Nginx
4. 磁盘⾼速缓存，页缓存（PageCache）
   - 把磁盘⽂件数据拷⻉「内核缓冲区」⾥，这个「内核缓冲区」实际上是磁盘⾼速缓存（PageCache）
   - 优点
      - 缓存最近被访问的数据
       读写磁盘相⽐读写内存的速度慢太多了，所以我们应该想办法把「读写磁盘」替换成「读写内存」，程序运⾏的时候，具有「局部性」，所以通常，刚被访问的数据在短时间内再次被访问的概率很⾼，于是我们可以⽤ PageCache 来缓存最近被访问的数据，当空间不⾜时淘汰最久未被访问的缓存
      - 预读功能
        机械磁盘寻道操作非常耗时，为了降低它的影响， PageCache 使⽤了「预读功能」。比如假设read每次只读32KB，但内核会把其后面的32~64KB也会拷贝到PageCache中
   - 缺点  
       - 在传输⼤⽂件（GB 级别的⽂件）的时候， PageCache 会不起作⽤，浪费 DMA 多做的⼀次数据拷⻉，造成性能的降低，即使使⽤了 PageCache 的零拷⻉也会损失性能
       - PageCache 由于⻓时间被⼤⽂件占据，其他「热点」的⼩⽂件可能就⽆法充分使⽤到 PageCache，于是这样磁盘读写的性能就会下降。在高并发情况下会造成严重的性能问题
   - 所以针对大文件传输不应该使用零拷贝技术
5. 大文件传输
   - 采用异步I/O（无阻塞）
     - 首先用户进程向内核发起异步I/O读，内核向磁盘发起IO请求，**立即返回**执行其他任务
     - 磁盘向内核发起IO终端信号，内核将数据从磁盘控制器缓存区拷贝到用户缓冲区，并通知用户进程读取成功
     - 用户进程处理IO
   - 直接I/O与缓存I/O
      使⽤异步 I/O 就意味着要绕开PageCache，绕开 PageCache 的 I/O 叫直接 I/O，使⽤ PageCache 的 I/O 则叫缓存 I/O。通常，对于磁盘，异步 I/O 只⽀持直接 I/O。
   - 直接I/O应用场景
