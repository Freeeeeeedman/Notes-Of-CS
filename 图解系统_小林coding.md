...
2. 中央处理器
   ...
   - 寄存器
     - 通⽤寄存器，⽤来存放需要进⾏运算的数据
     - 程序寄存器，⽤来存储 CPU 要执⾏下⼀条指令「所在的内存地址」
     - 指令寄存器，⽤来存放程序计数器指向的指令
3. 总线
...
1. 进程
   ...
   - 进程的控制结构
     ...
     - PCB组织方式
       - 通过链表的⽅式进⾏组织，把具有相同状态的进程链在⼀起，组成各种队列
         ...
         - 单核CPU只有一个运行指针
       - 索引方式
          同⼀状态的进程组织在⼀个索引表，索引表项指向相应的 PCB，不同状态对应不同的索引表
       - 因为进程增删操作多，所以一般采用链表
   - 进程的控制
     - 创建进程
   ...
   - 进程的上下文切换
     ...
     - 切换内容
         进程是由内核管理和调度的，所以进程的切换只能发⽣在内核态。进程的上下⽂切换不仅包含了虚拟内存、栈、全局变量等⽤户空间的资源，还包括了内核堆栈、寄存器等内核空间的资源。这些信息都保存、在PCB中
     - 切换场景
...
2. 线程
   ...
   - 线程的实现
     ...
     - 轻量级进程（Light-weight process， LWP）
       ...
       - LWP与普通进程的区别也在于它只有⼀个最⼩的执⾏上下⽂和调度程序所需的统计信息。LWP代表程序的执行线程，不需要那么状态信息
       ...
       - M : N ，即多个 LMP 对应多个⽤户线程
         ...
         - 线程是可以防止多个不同CPU内并行执行，LWP为执行线程，当然LWP内的线程就必须在同一CPU内
