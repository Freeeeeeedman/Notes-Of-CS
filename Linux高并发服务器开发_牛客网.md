...
2. B/S结构
   - 简介
     - B/S 结构（Browser/Server，浏览器/服务器模式），客户机上只要安装一个浏览器，如 Firefox 或 InternetExplorer，服务器安装 SQL Server、Oracle、MySQL 等数据库。浏览器通过 Web Server 同数据库进行数据交互。
   - 优点
...
#### 4.2 MAC地址，IP地址，端口
1. 网卡（NIC）
   - 网卡是一块被设计用来允许计算机在计算机网络上进行通讯的计算机硬件，又称为网络适配器或网络接口卡NIC。其拥有 MAC 地址，属于 OSI 模型的第 2 层，它使得用户可以通过电缆或无线相互连接。
...
2. MAC地址（Media Access Control Address）
   ...
   - 后 3 个字节，16进制数 AE-3C-40 代表该制造商所制造的
   - MAC 地址唯一的
   - ifconfig
...
5. 子网掩码（subnet mask）
   ...
   - 子网掩码也可划分子网管理网络。通过子网掩码，才能表明一台主机所在的子网与其他子网的关系，使网络正常工作。
   - 求网络ID： & 
   - 求主机ID： & (~ )
   - 192.168.100.10/24:表示子网掩码前24位都是1
6. 端口（port）
   - 简介
     - 端口是设备与外界通讯交流的出口。端口可分为虚拟端口和物理端口，其中虚拟端口指计算机内部或交换机路由器内的端口，不可见，是特指TCP/IP协议中的端口，是逻辑意义上的端口。例如计算机中的 80 端口、21 端口、23 端口等。
     - 物理端口又称为接口，是可见端口，计算机背板的 RJ45 网口，交换机路由器集线器等 RJ45 端口。电话使用 RJ11 插口也属于物理端口的范畴。
     - 端口是通过端口号来标记的，端口号只有整数，范围是从 0 到65535（2^16-1）。
   - 作用
     - A计算机访问B计算机内不同应用使用端口进行区分。不能用进程号区分，因为进程是动态分配的。
     - 一个应用程序可以有多个端口和不同的应用程序进行通信。
   - 类型
     1. 周知端口（Well Known Ports）
         - 周知端口是众所周知的端口号，范围从 0 到 1023，它们紧密绑定于一些特定的服务。例如 80 端口分配给 WWW 服务，21 端口分配给 FTP 服务，23 端口分配给Telnet服务等等。
         - 网络服务是可以使用其他端口号的，方法是在地址后面加上冒号“:”（半角），再加上端口号。“网址:8080”。
         - 有些系统协议使用固定的端口号，比如 139 端口专门用于 NetBIOS 与 TCP/IP 之间的通信，不能手动改变。
     2. 注册端口（Registered Ports）
         - 端口号从 1024 到 49151，它们松散地绑定于一些服务，分配给用户进程或应用程序，这些进程主要是用户选择安装的一些应用程序，而不是已经分配好了公认端口的常用程序。
...
2. OSI七层模型
   ...
   - 会话层
      建立数据传输的通路
   - 传输层
      定义了传输数据的协议和端口号，分段数据。建立，管理和维护端到端的连接
   - 网络层
      逻辑地址寻址，寻找IP地址，数据格式是报文
   - 数据链路层
      硬件地址寻址，MAC封装，将比特组合成字节进而组合成帧传输
   - 物理层
      定义物理设备标准，传输比特流
3. TCP/IP 四层模型
...
1. 定义
   ...
   - 最终体现为在网络上传输的数据包的格式
   - 协议分层定义，分层定义是为了使某一层协议的改变不影响其他层次的协议，上层协议使用下层协议提供的服务
2. 常见协议（具体构成看PDF）
   - 应用层常见协议
   ...
   - 网络层常见协议有
     ...
     - GMP 协议（Internet Group Management Protocol 因特网组管理协议）
   - 网络接口层常见协议
     - ARP协议（Address Resolution Protocol 地址解析协议）
     - RARP协议（Reverse Address Resolution Protocol 反向地址解析协议）

#### 4.5 网络通信的过程
1. 上层协议是如何使用下层协议提供的服务的呢？
2. 封装（encapsulation）
   - 应用程序数据在发送到物理网络上之前，将沿着协议栈从上往下依次传递。每层协议都将在上层数据的基础上加上自己的头部信息（有时还包括尾部信息），以实现该层的功能，这个过程就称为封装。
   - 四层传输，由上至下，应用程序数据->TCP报文段/UDP数据报->IP数据报->以太网帧
3. 分用（demultiplexing）
   - 当帧到达目的主机时，将沿着协议栈自底向上依次传递。各层协议依次处理帧中本层负责的头部数据，以获取所需的信息，并最终将处理后的帧交给目标应用程序。分用是依靠头部信息中的类型字段实现的。
   - 以太网头部有发送给上层哪个协议的数据，同样IP头部也有是发送给TCP还是UDP的信息，TCP/UDP头部也有端口号信息指定发送给哪个应用
4. ARP协议
   - 通过IP地址查找MAC地址
   - ARP请求：A广播发送，即给子网内所有机器发送
   - ARP应答：A发送的ARP包内包含目标设备B的IP，B检查如果一致则应答，向A发送包含B的MAC地址的ARP包
5. 疑问：是怎么找到哪个子网哪个设备的？为什么是接受数据时是现找到对应的MAC号再找到对应的IP？以及同一个IP有多个MAC地址呢？

#### 4.6 套接字Socket
1. socket简介
   - 所谓 socket（套接字），就是对网络中不同主机上的应用进程之间进行双向通信的端点的抽象。从所处的地位来讲，套接字上联应用进程，下联网络协议栈，是应用程序通过网络协议进行通信的接口，是应用程序与网络协议根进行交互的接口。
   - 是网络环境中进程间通信的 API，也是可以被命名和寻址的通信端点，使用中的每一个套接字都有其类型和一个与之相连进程。通信时其中一个网络应用程序将要传输的一段信息写入它所在主机的 socket 中，该 socket 通过与网络接口卡（NIC）相连的传输介质将这段信息送到另外一台主机的 socket 中，使对方能够接收到这段信息。
   - socket 是由 IP 地址和端口结合的，提供向应用层进程传送数据包的机制。
   - 在 Linux 环境下，用于表示进程间网络通信的特殊文件类型。本质为内核借助缓冲区形成的伪文件。我们可以使用文件描述符引用套接字。Linux 系统将其封装成文件的目的是为了统一接口，使得读写套接字和读写文件的操作一致。
   - 优点：封装了底层的通信机制，我们只需要调用API即可
   - 套接字通信分两部分：
...
2. 为什么至少是三次握手而不是两次握手？
   - 客户端要确定自己的收发和服务端的收发都没问题
   - 服务端也要确定自己的收发和客户端的收发都没问题
   - 最后一次握手服务端确定了客户端的收没问题
   - 四次握手也可以
3. 序号seq与确认序号ack
   - 给字节流每个字节分配一个序号，这里的序号比如一个报文段的序号为300，数据部分为100字节，则下一个报文段的序号为400，不加上首部的字节数，即300（100）
   - 当发送的报文段有SYN/FIN标志位则还要**再**+1，ACK不加1
   - 为了保证数据的完整和顺序
   - 怎么指定序号的？
4. SYN报文不能携带数据，ACK报文分情况，FIN报文可以携带数据
...
6. 流程
   ...
   - 第二次握手：
     ...
     2. 服务器会回发一个确认序号： ack=客户端的序号 + SYN/FIN(按一个字节算)
     3. 服务器端会向客户端发起连接请求： SYN=1
     4. 服务器会生成一个随机序号：seq = K
   - 第三次握手：
     1. 客户端应答服务器的连接请求：ACK=1
     2. 客户端回复收到了服务器端的数据：ack=服务端的序号 + SYN/FIN(按一个字节算)
   - 注意前两次握手不可以携带数据，交换的报文段只有TCP的首部。最后一次握手可以携带数据的
...
3. 多进程实现
   ...
   - 注意：
      ```
        ...
        要带上结束符，sprintf是不会将\n写入的，所以要+1，这样也可以防止发送上次残留的数据（有点蠢，还是每次再次发送前清除数据再输入）
      ```
      ```
      软中断，当设置信号捕捉函数时，执行回调函数会造成软中断，回到except会造成其不阻塞，所以就需要跳过这次循环
         if(cfd == -1) {
            if(errno == EINTR) {
                continue;;  
            }
...
3. int select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds, struct timeval *timeout);
   - 参数：
     ...
     - readfds : 要检测的文件描述符的读的集合，委托内核检测哪些文件描述符的读的属性
       - sizeof(fd_set) = 128 1024位，1024个文件描述符，fd_set集合存放文件描述符
       - 一般检测读操作
...
#### 5.1 网络IO：阻塞与非阻塞，同步与异步
1. IO的两个阶段
   - 数据就绪：根据系统IO操作的就绪状态， TCP接受缓存区
     - 阻塞
       - 调用IO方法的线程进入阻塞状态
       - recv
     - 非阻塞
       - 不会改变线程的状态，通过返回值判断
       - ret == -1 出错了（EINTR, EAGAIN, EWOULDBLOCK）
       - ret == 0,读到数据末尾，对方连接关闭
       - ret > 0, 读取到了多少的数据
   - 数据读写：根据应用程序和内核的交互方式， 应用程序
     - 同步
       - TCP接受缓冲区搬运到自定义的buf中，搬完才返回，进行下面的代码
     - 异步
       - 异步IO接口： sockfd, buf, 通知方式：sigio信号
       - 调用完异步IO接口，代码就马上往下执行，系统搬运完数据通过信号通知用户
   - 陈硕：在处理 IO 的时候，阻塞和非阻塞都是同步 IO，只有使用了特殊的 API 才是异步 IO
       - 异步IO： Linux：AIO, Windows：IOCP
       - 异步IO一般和非阻塞结合使用
       - 同步IO: IO多路复用select，poll，epoll
       - 虽然是内核帮我们检测哪些缓冲区发生变化，但是还是要靠我们自己调用同步IO：read，recv来接受数据。记住只有调用了异步IO的API才是异步IO
2. 概念
...
3. IO复用（IO multiplexing）
   ...
   - 在单线程/单进程的情况下可检测多个客户端事件
   - select，poll，epoll可以设置非阻塞
