...
5. 如何查看MySQL的客户端的连接信息？如何查看 MySQL 服务被多少个客户端连接了？
    show processlist
6. 空闲连接会一直占用着吗？
...
16. InnoDB是怎么解决幻读问题的？
    ...
    - 对于当前读，比如SELECT ... FOR UPDATE，update，insert，delete等是采用next-key锁来解决。next-key锁就是记录锁和间隙锁的组合。不过有时候next-key会退化为当前锁（引导面试官提问！）
        记录锁，会锁住本行的记录；而间隙锁，锁的就是两个值之间的空隙，以防止其他事务在这个空隙间插入新的数据，从而避免幻读现象。
...
5. 备份数据库时如何避免全局锁的问题？/备份数据库时如何避免业务收到影响？/备份数据库时可以更新数据库吗？
