...
#### 2. 架构问题
1. 说一说MySQL的基础架构
    MySQL可以分为服务层和存储引擎层。Server层包括连接器、 查询缓存、 分析器、 优化器、 执行器，涵盖MySQL的大多数核心服务功能， 以及所有的内置函数（如日期、 时间、 数学和加密函数等） ， 所有跨存储引擎的功能都在这一层实现， 比如存储过程、 触发器、 视图等
...
4. 如何查看MySQL的客户端的连接信息？
    show processlist
5. 什么是数据库的长连接与短连接？
    长连接是指连接成功后， 如果客户端持续有请求， 则一直使用同一个连接。 短连接则是指每次执行完很少的几次查询就断开连接， 下次查询再重新建立一个。
    每次连接开销大，应该尽量使用长连接
6. 数据库长连接会导致什么问题？
    会导致MySQL占用内存涨得特别快，因为MySQL在执行过程中临时使用的内存是管理在连接对象里面的。 这些资源会在连接断开的时候才释放。MySQL占用内存过大时会导致系统强行杀掉，即系统强行杀掉。
7. 如何解决长连接的问题？
   - 定期断开长连接。 
   - 执行mysql_reset_connection来重新初始化连接资源。 这个过程不需要重连和重新做权限验证，但是会将连接恢复到刚刚创建完时的状态。
8. 说一说MySQL的查询缓存
...
10. 如果表T中没有字段k， 而你执行了这个语句 select * from Twhere k=1, 那肯定是会报“不存在这个列”的错误： “Unknown column ‘k’ in ‘where clause’”。 你觉得这个错误是在我们上面提到的哪个阶段报出来的呢？
    分析器的词法分析
...
1. 不同数据结构的特点
   - 哈希表
      - 键值对存储数据的结构，通过哈希函数对应。
      - 优点：在没有哈希冲突时，搜索的时间复杂度是O(1)
      - 缺点：只适用于等值查询的场景。产生哈希冲突时，如果通过拉链法解决。由于链表中的数据不是有序加入的，会导致区间查询需要全部扫描，很低
   - 有序数组
      - 优点：在等值查询和范围查询场景中的性能就都非常优秀。通过二分法查询时间复杂度是O(log(N))。
      - 缺点：只适用于静态存储引擎即不会再修改的数据。在更新数据时需要挪动大量数据。
   - 二叉搜索树（B树）
      - 优点：查询复杂度O(log(N))，同时为了维持其是平衡二叉树，更新的时间复杂度也是O(log(N))。
      - 缺点：索引不止存在内存中， 还要写到磁盘上。从磁盘中读取一个数据时，二叉搜索树需要多次随机访问大量的数据块，时间开销太大。
   - N叉树（B+树）
     不需要像二叉搜索树一样多次随机访问数据块，在读写上有着性能优点，适配磁盘的访问模式， 已经被广泛应用在数据库引擎中
...
7. 在一个市民信息表上， 是否有必要将身份证号和名字建立联合索引？
    如果有一个高频请求， 要根据市民的身份证号查询他的姓名， 这个联合索引就有意义了。 它可以在这个高频请求上用到覆盖索引， 不再需要回表查整行记录， 减少语句的执行时间。
8. 说一说最左前缀原则
    索引项是按照索引定义里面出现的字段顺序排序的。只要满足最左前缀， 就可以利用索引来加速检索。 这个最左前缀可以是联合索引的最左N个字段， 也可以是字符串索引的最左M个字符。
9. 说一说在建立联合索引的时候， 如何安排索引内的字段顺序
