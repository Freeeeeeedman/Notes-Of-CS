...
11. 如何排查网络问题？
    ...
    - CPU/内存：dmidecode看硬件信息，top/htop/ps看进程CPU占用，vmstat（虚拟内存统计CPU内存IO），还有strace分析进程的系统调用情况
    - IO：iostat查看IO吞吐量
12. 用户如何与你的Web服务器进行通信(前后端如何通信)
...
3. select，poll，epoll
   ...
   - 其次epoll不需要拷贝整个文件描述符表
        epoll在内核⾥使⽤红⿊树来跟踪进程所有待检测的⽂件描述字，把需要监控的 socket 通过epoll_ctl() 函数加⼊内核中的红⿊树⾥，红⿊树增删查⼀般时间复杂度是O(logn) ，就不需要每次都拷贝一整个socket集合，只需要传⼊⼀个待检测的 socket，减少了拷贝开销和内存占用
   - 最后epoll不遍历所有的文件描述符
        使⽤异步事件驱动的机制，内核⾥维护了⼀个双向链表来记录就绪事件，当某个 socket 有事件发⽣时，内核会将其加⼊到这个就绪事件链表中、当⽤户调⽤ epoll_wait() 函数时，只会返回有事件发⽣的⽂件描述符的个数，以及通过传出参数传出哪些文件描述符有事件，不需要轮询扫描整个 socket 集合，这样epoll遍历的时间复杂度为O(1)。
   - 当监测的fd数目较小，且各个fd都比较活跃，事件触发比较频繁，建议使用select或者poll,因为这时回调函数会被频繁触发，而回调函数涉及系统调用来传递地址，导致用户态和内核态频繁切换，效率就不一定高了
   - 当监测的fd数目非常大，但活跃的连接数量不多，这个时候使用epoll能够明显提升性能
16. 说一说epoll的水平触发模式和边缘触发模式
    - LT水平触发模式
       - 被监控的文件描述符有可读事件，只要内核缓冲区内还有数据，比如读缓冲区太小不能一次读完，就会一直触发，**epoll_wait()会一直通知程序进行操作，确保读完**
       - 对于可写事件，LT模式下只要内核缓冲区可写就一直会触发，所以要在数据发送完成后**移除检测可写事件**
       - 和阻塞或者非阻塞文件描述符相使用，epoll_wait()通知有读事件后，通过recv()数据返回
    - ET边缘触发模式
       - 被监控的文件描述符有可读事件，如果读缓冲区过小不能一次读完，都**只会触发一次**，即epoll_wait()只会通知一次程序来进行操作，直到下一次新的数据到达。**所以要通过while来循环读取数据，确保读完**。
       - 对于可写事件，ET模式下如果内核缓冲区可写（有空位）则只会通知一次，所**以如果send()发送数据不能一次发送完成时，则必须再次注册可写事件。**
...
7.  listenfd阻塞还是非阻塞？是ET还是LT
    - 最好采用的是非阻塞+ET
      - 如果设置阻塞，在服务器调用accept之前客户端中止连接发送RST报文，服务器将连接从全连接队列中移除，之后accept就会阻塞直到新连接的到达
      - ET模式效率更高，但必须采用while循环来accept()来确保能获取到所有的新连接   
8. epoll
...
#### 日志相关问题
1. **如何实现日志系统？**
      初始化服务器时，利用单例模式初始化日志系统，根据配置文件确认是同步还是异步写入的方式。如果是同步写入，则每次需要写日志的时 候，就直接调用日志类中的公有函数对日志文件进行加锁写入。而如果是异步写入的方式，就需要创建一个线程以及阻塞队列。每次需要写日志的时候，将日志信息加入阻塞队列。再由新的线程通过生产者与消费者的模式写入日志文件。
2. 为什么要异步？和同步的区别是什么？
