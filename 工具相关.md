...
4. 基本操作
   ...
   - 回退版本 
     ...
     - git reset --soft
       - 重置HEAD，保留暂存区和工作区。修改内容还在，变成未add的状态。
...
6. 变基
   ...
   - 风险
      ...
      - 因为变基操作的实质是丢弃一些现有的提交，然后相应地新建一些内容一样但实际上不同的提交。当别人进行合并提交后回滚改为变基，会多出来内容相同但是实际不同的提交，而我们再指向git pull会合并两次相同内容的提交。并且导致由于变基丢失的提交又回来了，使得整个历史非常混乱。
7. 标签
   - 创建带注解的标签
      git tag -a v1.0 -m ''
   - 追加标签
...
8. 远程仓库
   ...
   - 拉取更新
     ...
     - git log -p FETCH_HEAD git log -p FETCH_HEAD
     - 最终要将拉取的更新合并到当前本地分支
